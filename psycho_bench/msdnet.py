...
class ClassifierModule(nn.Module):
    def __init__(self, m, channel, num_classes):
        ...
        # Build the Linear lazily once we know the true flattened dim
        self.linear = None

    def forward(self, x):
        # x is a list of feature maps at different scales; MSDNet uses x[-1]
        res = self.m(x[-1])
        res = res.view(res.size(0), -1)  # (B, D_flat)

        if self.linear is None:
            in_features = res.size(1)
            self.linear = nn.Linear(in_features, self.num_classes).to(res.device)
